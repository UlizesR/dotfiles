...
-- Wayland is a Linux-only concept; leaving this true unconditionally was
-- harmless (WezTerm ignores it elsewhere) but misleading to read on a
-- macOS-targeting config, so it's scoped explicitly.
...
-- Hyprland (a tiling compositor) already draws its own borders/gaps, so
-- WezTerm's own window chrome is redundant there. On macOS, "RESIZE" keeps
-- the resize border without a title bar, which is the more common choice.
...
-- Font rendering: FreeType tuning is a Linux-specific knob. Forcing it on
-- macOS as well doesn't make the two platforms render more alike -- macOS's
-- native CoreText-based rasterizer and Linux's FreeType are fundamentally
-- different text-rendering pipelines, so identical settings still produce
-- different-looking glyphs. This just makes sure each platform uses its
-- best native path instead of an override tuned for the other one.
if not is_macos then
